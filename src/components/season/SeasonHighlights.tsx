  ...
  return (
    <div className="bg-white border border-navy/10 rounded-lg p-3 shadow-sm">
      ...
      {records.map((r, i) => {
        ...
        return (
          <div key={`${r.teamSlug}-${r.week}-${i}`} className={`flex justify-between text-sm font-body py-0.5 px-1.5 -mx-1.5 rounded ${isNew ? 'bg-amber-100' : ''}`}>
  ...
  return (
    <div className="bg-white border border-navy/10 rounded-lg p-3 shadow-sm">
      ...
      {records.map((r, i) => {
        ...
        const isNew = r.week === currentWeek;
        return (
          <div key={`${r.slug}-${r.week}-${i}`} className={`flex justify-between text-sm font-body py-0.5 px-1.5 -mx-1.5 rounded ${isNew ? 'bg-amber-100' : ''}`}>
            <span className="truncate mr-2">
