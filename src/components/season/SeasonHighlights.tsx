...
export function SeasonHighlights({ weeklyScores }: Props) {
  ...
  const turkeyListAll: CountEntry[] = Array.from(turkeyMap.values())
    .sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));
  const topTurkeys = topWithTies(turkeyListAll, 5, e => e.count);

  // --- 200 Games (season cumulative) ---
  const twoHundredMap = new Map<number, { name: string; slug: string; count: number }>();
  for (const b of bowlers) {
    let count = 0;
    ...
    if (count > 0) {
      const cur = twoHundredMap.get(b.bowlerID) ?? { name: b.bowlerName, slug: b.bowlerSlug, count: 0 };
      cur.count += count;
      twoHundredMap.set(b.bowlerID, cur);
    }
  }
  const twoHundredListAll: CountEntry[] = Array.from(twoHundredMap.values())
    .sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <TeamRecordCard title="High Team Hcp Series" records={topTeamHcp} />
        <TeamRecordCard title="High Team Scratch Series" records={topTeamScratch} />
      </div>

      {/* Individual series records */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {topMenSeries.length > 0 && <IndividualRecordCard title="High Men's Scratch Series" records={topMenSeries} />}
        {topWomenSeries.length > 0 && <IndividualRecordCard title="High Women's Scratch Series" records={topWomenSeries} />}
      </div>
...
function TeamRecordCard({ title, records }: { title: string; records: TeamRecord[] }) {
  ...
  return (
    <div className="border border-navy/10 rounded-lg p-3">
      ...
      {records.map((r, i) => {
        ...
        return (
          <div key={`${r.teamSlug}-${r.week}-${i}`} className="flex justify-between text-sm font-body py-0.5">
...
function IndividualRecordCard({ title, records }: { title: string; records: SeasonRecord[] }) {
  if (records.length === 0) return null;
  const topVal = records[0].value;
  return (
    <div className="border border-navy/10 rounded-lg p-3">
      ...
      {records.map((r, i) => {
        const isTop = r.value === topVal;
        return (
          <div key={`${r.slug}-${r.week}-${i}`} className="flex justify-between text-sm font-body py-0.5">
            <span className="truncate mr-2">
  ...
  return (
    <div className="border border-navy/10 rounded-lg p-3">
      ...
      {entries.map((e, i) => {
        ...
        return (
          <div key={`${e.slug}-${i}`} className="flex justify-between text-sm font-body py-0.5">
            <Link href={`/bowler/${e.slug}`} className={`text-navy hover:text-red-600 transition-colors truncate mr-2 ${isTop ? 'font-bold' : ''}`}>
              {e.name}
            </Link>
            <span className={`tabular-nums shrink-0 ${isTop ? 'font-bold text-navy' : 'text-navy/60'}`}>{e.count}</span>
          </div>
        );
      })}
      {tiedCount > 0 && (
