...
import type { BowlerNode, BowlerPair, TeamViz } from './teams-data';
...
export const AD_PAD = 30;
export const AD_BASELINE = 195;
export const AD_H = 300;
...
export function nodeColor(t: number): string {
  const sat = Math.round(75 - t * 65);
  const lit = Math.round(58 + t * 36);
  return `hsl(220,${sat}%,${lit}%)`;
}

export function ArcDiagram({ bowlers, pairs, nightsMap, champCounts, captainID, title, selectedId, selectedTeam, onSelect, sort, showLegend, maxNights, allTeamsForPanel }: {
  bowlers: BowlerNode[];
  pairs: BowlerPair[];
  ...
  onSelect: (id: number | null, team: string | null) => void;
  sort: 'firstSeason' | 'totalNights';
  showLegend?: boolean;
  maxNights: number;
  allTeamsForPanel: TeamViz[];
}) {
  ...
  const isPanelExpanded = selectedId !== null && selectedTeam === title;
  const focusId = (selectedInThisTeam ? selectedId : null) ?? hovNodeId;

  return (
    <div>
      <div style={{ position: 'relative' }}>
        <svg width={AD_W} height={AD_H} style={{ display: 'block', overflow: 'visible' }} onMouseLeave={() => { setHovArc(null); setHovNodeId(null); }}>
          ...
          {[...pairs].reverse().map(pair => {
            ...
            return (
              <path
                ...
                onMouseEnter={isConnected ? () => setHovArc(pair) : undefined}
          ...
          {sortedBowlers.map(b => {
            ...
            return (
              <g key={b.id} style={{ cursor: 'pointer' }}
                onClick={() => onSelect(selectedId === b.id && selectedTeam === title ? null : b.id, selectedId === b.id && selectedTeam === title ? null : title)}
                onMouseEnter={() => setHovNodeId(b.id)}
      ...
      {isPanelExpanded && selectedId !== null ? (() => {
        const teamsForBowler = allTeamsForPanel
          .filter(t => t.bowlers.some(b => b.id === selectedId))
          .sort((a, b) => (b.nightsMap[selectedId] ?? 0) - (a.nightsMap[selectedId] ?? 0));
        const bowlerName = teamsForBowler[0]?.bowlers.find(b => b.id === selectedId)?.name ?? '';
        const champs = champCounts[selectedId] ?? 0;
        return (
          <div style={{ borderTop: '1px solid #1e2d3d', padding: '10px 30px 14px', background: '#091624' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              <span style={{ color: '#fbbf24', fontWeight: 700, fontSize: 13 }}>{bowlerName}</span>
              {champs > 0 && <span style={{ color: '#f59e0b', fontSize: 14 }}>{'♛'.repeat(champs)}</span>}
              <button onClick={() => onSelect(null, null)} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#475569', cursor: 'pointer', fontSize: 16, lineHeight: 1, padding: '0 2px' }}>&#x2715;</button>
            </div>
        ...
        <div style={{ height: 28, display: 'flex', alignItems: 'center', paddingLeft: AD_PAD, gap: 12 }}>
          {hovArc !== null && (() => {
            const b1 = bowlers.find(b => b.id === hovArc.id1);
            const b2 = bowlers.find(b => b.id === hovArc.id2);
            if (!b1 || !b2) return null;
            return <>
              <span style={{ color: '#f8fafc', fontWeight: 700, fontSize: 12 }}>{b1.name} &amp; {b2.name}</span>
              <span style={{ color: '#60a5fa', fontSize: 12 }}>{hovArc.coNights} nights together</span>
            </>;
          })()}
          {hovArc === null && hovNodeId !== null && (() => {
            const b = bowlers.find(b => b.id === hovNodeId);
            if (!b) return null;
            const nights = nightsMap[hovNodeId] ?? 1;
            return <>
              <span style={{ color: '#f8fafc', fontWeight: 700, fontSize: 12 }}>{b.name}{(champCounts[b.id] ?? 0) > 0 && <span style={{ color: '#f59e0b' }}>{' '}{'♛'.repeat(champCounts[b.id])}</span>}</span>
              <span style={{ color: '#60a5fa', fontSize: 12 }}>{nights} nights &bull; since {SEASON_LABELS[b.firstSeason]}</span>
              <span style={{ color: '#475569', fontSize: 11 }}>click to pin</span>
            </>;
          })()}
        </div>
      )}
