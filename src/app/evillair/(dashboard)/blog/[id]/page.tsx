...
export default function BlogEditorPage({
  ...
  const loadPost = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/evillair/blog/${postId}`);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <button
          onClick={() => router.push('/evillair/blog')}
        ...
        <div className="flex items-center gap-3">
          ...
          {slug && (
            <button
              onClick={async () => {
                ...
                window.open(`/api/evillair/draft?slug=${encodeURIComponent(slug)}`, '_blank');
              }}
          ...
          {post.publishedAt ? (
            <>
              ...
              <button
                onClick={async () => {
                  const res = await fetch('/api/evillair/new-blog-badge', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
          ...
          <div>
            ...
            <input
              type="text"
              value={cardFocalY}
              onChange={(e) => setCardFocalY(e.target.value)}
              className="w-full px-3 py-2 rounded-md border border-navy/20 font-body text-sm focus:outline-none focus:border-navy/40"
              placeholder="0.3 (optional, falls back to hero focal)"
            />
          </div>
        </div>
      </details>
