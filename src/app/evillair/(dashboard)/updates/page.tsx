...
export default function AdminUpdatesPage() {
  ...
  async function handleDelete(id: number) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/evillair/updates/${id}`, { method: 'DELETE' });
  ...
  async function saveEdit() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/evillair/updates/${editingId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function loadSuggestions() {
    ...
    try {
      ...
      const res = await fetch(`/api/evillair/updates/suggest?since=${since}`);
      if (!res.ok) {
        const d = await res.json();
