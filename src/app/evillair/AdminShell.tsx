...
const NAV_ITEMS: NavItem[] = [
  ...
  { kind: 'divider' },
  { kind: 'link', label: 'Updates', href: '/evillair/updates', icon: UpdatesIcon, adminOnly: true },
  { kind: 'link', label: 'Announcements', href: '/evillair/announcements', icon: AnnouncementsIcon, adminOnly: true },
  { kind: 'divider' },
  { kind: 'link', label: 'Email', href: '/evillair/email', icon: EmailNavIcon, adminOnly: true },
  { kind: 'link', label: 'Captains', href: '/evillair/captains', icon: CaptainsIcon, adminOnly: true },
  { kind: 'link', label: 'Bowlers', href: '/evillair/bowlers', icon: BowlersIcon, adminOnly: true },
  { kind: 'divider' },
  { kind: 'link', label: 'Instagram', href: '/evillair/instagram', icon: InstagramIcon, adminOnly: true },
];
...
export function AdminShell({ children, role = 'admin' }: { children: React.ReactNode; role?: string }) {
  ...
  async function handleLogout() {
    await fetch('/api/evillair/auth/logout', { method: 'POST' });
    router.push('/evillair/login');
  }
    ...
    <div className="fixed inset-0 z-[100] flex bg-cream">
      ...
      <aside className="hidden md:flex md:flex-col md:w-56 bg-navy text-cream">
        <div className="px-5 py-6 border-b border-white/10">
          <Link href="/evillair" className="font-heading text-xl text-cream">
            Splitzkrieg
          </Link>
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-navy text-cream">
          <Link href="/evillair" className="font-heading text-lg text-cream">
            Splitzkrieg Admin
          </Link>
          <button
